...
static uint32_t
doser_volumeToTimeMs(Doser *doser,
                     float volume) { // calculates the time to turn on pump to
                                     // dispense X amount of liquid in mL
                                     // error checking
...
void doser_start(Doser *doser, float volumeMl) {
  ...
  {
    printf("Doser or pump null\r\n");
    return;
  }
  if (volumeMl <= 0.0f) {
    printf("Volume less than 0\r\n");
    return;
  }
